...
Azureソリューションを構築、デプロイ、監視するために必要なすべてを含む完全なToDoアプリケーション。このアプリケーションは、IaCプロバイダーとしてBicepを使用し、WebアプリケーションにReact.js、APIにNode.js、ストレージにAzure Cosmos DB API for MongoDB、監視とログ記録にAzure Monitorを使用して、Azure Developer CLI（azd）を使用してAzure上で素早く起動できます。アプリケーションコード、ツール、パイプラインが含まれており、自分自身のソリューションを作成する際に基盤となり、カスタマイズすることができます。

AzureでToDoアプリを立ち上げて、完全に機能するWebアプリをデプロイします。後のステップでは、パイプラインの設定方法とアプリケーションの監視方法を確認します。

<img src="assets/web.png" width="75%" alt="Screenshot of deployed ToDo app">

<sup>ToDoアプリ デプロイ スクリーンショット</sup>
